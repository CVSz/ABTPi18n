...
try:
    ...
    print("ERROR: gdown is not installed. Please run: pip install gdown", file=sys.stderr)
    sys.exit(1)
...
def download_drive_folder(folder_id: str, output_dir: str = "external/drive_assets") -> bool:
    """
    ...
    try:
        ...
        # Download the folder
        gdown.download_folder(url, output=str(output_path), quiet=False, remaining_ok=True)
        ...
        logger.error("Make sure the folder is shared as 'Anyone with the link can view'")
        return False
