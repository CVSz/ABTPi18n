...
def load_mapping_config(config_path: str) -> Dict[str, Any]:
    ...
    if not config or 'rules' not in config:
        raise ValueError(f"Invalid mapping config: must contain 'rules' key")
...
def integrate_assets(
    ...
    for idx, rule in enumerate(rules, 1):
        ...
        logger.info(f"Rule {idx}: Processing patterns {patterns} -> {dest_dir}")
        ...
        for source_file in matched_files:
            ...
            logger.info(f"  {action}: {source_file.relative_to(assets_path)} -> {dest_file}")
    ...
    logger.info(f"Integration complete: {files_processed} files processed, {files_copied} files copied")
