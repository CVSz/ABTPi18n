...
class StrategyRegistry:
    ...
    def register(cls, strategy_cls: Type[Strategy]) -> None:
        ...
        if not hasattr(strategy_cls, 'name'):
            raise ValueError(f"Strategy class {strategy_cls.__name__} must define a 'name' attribute")
    ...
    def create(cls, name: str) -> Strategy:
        ...
        if name not in cls._strategies:
            raise ValueError(f"Strategy '{name}' not registered. Available: {cls.list_names()}")
