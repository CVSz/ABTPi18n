...
def load_external_strategies(external_dir: str = "strategies/external") -> List[str]:
            ...
            for attr_name in dir(module):
                ...
                if (isinstance(attr, type) and 
                    ...
                    if attr.name not in StrategyRegistry.list_names():
                        ...
                        logger.info(f"Auto-registered strategy '{attr.name}' from {py_file.name}")
            
        except Exception as e:
            logger.error(f"Error loading external strategy from {py_file}: {e}", exc_info=True)
            continue
    ...
    if loaded_strategies:
        logger.info(f"Successfully loaded {len(loaded_strategies)} external strategies: {loaded_strategies}")
    else:
