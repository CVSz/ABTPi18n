    ...
    def execute(self, ticker_data: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(closes) < self.window + 5:
            return {
                "signal": "HOLD",
                "confidence": 0.0,
                "meta": {"reason": f"Insufficient data: need {self.window + 5}, got {len(closes)}"}
            }
        ...
        if pd.isna([current_price, current_ma, current_std, current_z]).any():
            return {
                "signal": "HOLD",
                "confidence": 0.0,
                "meta": {"reason": "Insufficient historical data for calculation (NaN values)"}
            }
        ...
        z_confidence = min(abs(current_z) / self.z_entry, 1.0) if self.z_entry > 0 else 0.0
